...
#define ASYNCFUTURE_ERROR_ARGUMENT_MISMATCHED                                                             \
    "The callback function is not callable. The input argument doesn't match with the observing QFuture " \
    "type"
...
#define ASYNC_FUTURE_CALLBACK_STATIC_ASSERT(Tag, Completed)                                     \
    ...
    static_assert(!(std::is_same<void, T>::value && Private::arg_count<Completed>::value >= 1), \
...
namespace Private {
    ...
    /* It is an additional to the original function_traits to handle non-const function (with mutable keyword
     * lambda). */
    ...
    struct is_callable
    {
        enum
        {
            value = (arg_count<Functor>::value == 1) && (!std::is_same<void, T>::value)
    ...
    {
        QObject tmp;
        QObject::connect(&tmp, &QObject::destroyed, QCoreApplication::instance(), std::move(func),
                         Qt::QueuedConnection);
    }
    ...
    void watch(QFuture<T> future, const QObject* owner, const QObject* contextObject, Finished finished,
               Canceled canceled, Progress progress, ProgressRange progressRange)
    {
        ...
        if (owner)
        {
            ...
            QObject::connect(owner, &QObject::destroyed, watcher, [watcher]() { delete watcher; });
        }
        ...
        if (contextObject)
        {
            ...
            QObject::connect(watcher, &QFutureWatcher<T>::progressValueChanged, contextObject,
                             [=](int value) { progress(value); });

            QObject::connect(watcher, &QFutureWatcher<T>::progressRangeChanged, contextObject,
                             [=](int min, int max) { progressRange(min, max); });
        }
        else
        {
     ...
     *
     * 2) Its member function do not use <T> to avoid to use template specialization to handle <void>. Type
     * checking should be done by user classes (e.g Deferred)
     *
     */
    ...
    public:
        ...
        void track(QFuture<ANY> future)
        {
            ...
            QObject::connect(watcher, &QFutureWatcher<ANY>::progressRangeChanged, this,
                             [=](int min, int max) {
                                 if (thiz.isNull())
                                 {
                                     return;
            ...
            QObject::connect(watcher, &QFutureWatcher<ANY>::started, this, [=]() { thiz->reportStarted(); });

            QObject::connect(watcher, &QFutureWatcher<ANY>::paused, this, [=]() { thiz->setPaused(true); });

            QObject::connect(watcher, &QFutureWatcher<ANY>::resumed, this, [=]() { thiz->setPaused(false); });
        ...
        void complete(QFuture<T> future)
        {
            ...
            watch(
                future, this, nullptr, onFinished, onCanceled, [](int) {}, [](int, int) {});
            ...
            watch(
                this->future(), this, nullptr, []() {}, pushCancel, [](int) {}, [](int, int) {});
        ...
        void cancel(QFuture<ANY> future)
        {
            ...
            auto onCanceled = [=]() { decWeakRefCount(); };
    ...
    public:
        ...
        void addFuture(const QFuture<T> future)
        {
            ...
            Private::watch(
                ...
                progressFunc, progressRangeFunc);
        }
    ...
    private:
        ...
        {
            int max =
                std::accumulate(futures.begin(), futures.end(), 0,
                                [](int current, const FutureInfo* info) { return info->max + current; });
        ...
        void updateProgress()
        {
            int value =
                std::accumulate(futures.begin(), futures.end(), 0,
                                [](int current, const FutureInfo* info) { return info->value + current; });
    ...
    public:
        ...
        void bind(QObject* source, Method pointToMemberFunction)
        {
            ...
            conn = QMetaObject::connect(source, method.methodIndex(), this, memberOffset,
    ...
    static QFuture<DeferredType> execute(QFuture<T> future, const QObject* contextObject,
                                         Completed onCompleted, Canceled onCanceled)
    {
        ...
        watch(
            future, contextObject, contextObject,
            [=]() {
                try
                {
            ...
            [=](int progressValue) { defer->setParentProgressValue(progressValue); },
            [=](int min, int max) { defer->setParentProgressRange(min, max); });
        ...
        watch(
            defer->future(), contextObject, contextObject, []() {}, // onComplete
...
public:
    ...
    typename std::enable_if<
        !Private::future_traits<typename Private::function_traits<Completed>::result_type>::is_future,
        Observable<typename Private::function_traits<Completed>::result_type>>::type
    context(const QObject* contextObject, Completed functor)
    {
        ...
        return _context<typename Private::function_traits<Completed>::result_type,
                        typename Private::function_traits<Completed>::result_type>(contextObject, functor,
                                                                                   []() {});
    }
    ...
    typename std::enable_if<
        Private::future_traits<typename Private::function_traits<Completed>::result_type>::is_future,
        Observable<typename Private::future_traits<
            typename Private::function_traits<Completed>::result_type>::arg_type>>::type
    context(const QObject* contextObject, Completed functor)
    {
        ...
        return _context<typename Private::future_traits<
                            typename Private::function_traits<Completed>::result_type>::arg_type,
                        typename Private::function_traits<Completed>::result_type>(contextObject, functor,
                                                                                   []() {});
    }
    ...
    context(const QObject* contextObject, Completed onCompleted, Canceled onCanceled)
    {
        ...
        return _context<typename Private::function_traits<Completed>::result_type,
                        typename Private::function_traits<Completed>::result_type>(contextObject, onCompleted,
                                                                                   onCanceled);
    }
    ...
    typename std::enable_if<!Private::ret_type_is_future<Completed>::value,
                            Observable<typename Private::RetType<Completed>>>::type
    subscribe(Completed onCompleted, Canceled onCanceled)
    {
        ...
        return _subscribe<typename Private::function_traits<Completed>::result_type,
                          typename Private::function_traits<Completed>::result_type>(onCompleted, onCanceled);
    }
    ...
    subscribe(Completed onCompleted)
    {
        ...
        return _subscribe<typename Private::function_traits<Completed>::result_type,
                          typename Private::function_traits<Completed>::result_type>(onCompleted, []() {});
    }

    template <typename Completed, typename Canceled>
    typename std::enable_if<Private::ret_type_is_future<Completed>::value,
                            Observable<typename Private::function_traits<Completed>::future_arg_type>>::type
    subscribe(Completed onCompleted, Canceled onCanceled)
    {
        ...
        return _subscribe<typename Private::future_traits<
                              typename Private::function_traits<Completed>::result_type>::arg_type,
                          typename Private::function_traits<Completed>::result_type>(onCompleted, onCanceled);
    }
    ...
    void onFinished(std::function<void()> func)
    {
        auto runOnMainThread = [=]() {
            ...
            QObject tmp;
            QObject::connect(&tmp, &QObject::destroyed, QCoreApplication::instance(), func,
                             Qt::QueuedConnection);
#else
...
private:
    ...
    Observable<ObservableType> _context(const QObject* contextObject, Completed onCompleted,
                                        Canceled onCanceled)
    {

        auto future =
            Private::execute<ObservableType, RetType>(m_future, contextObject, onCompleted, onCanceled);
...
public:
    ...
    void complete(QFuture<QFuture<ANY>> future)
    {
        ...
        static_assert(Private::False<ANY>::value,
                      "Deferred<void>::complete(QFuture<QFuture<ANY>>) is not supported");
    }
